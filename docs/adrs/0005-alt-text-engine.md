...
1. Claude Vision, via a dedicated `AltTextService`/`ClaudeVisionAdapter` — a real API call per figure, independent of the `ocr`/`finalize_tags` stage's engine.
...
- `AltTextService` depends on the Anthropic API (`ClaudeVisionAdapter`) — a per-figure network call, with the cost and latency that implies, gated by its own `RUN_ALT_TEXT` setting like every other stage.
